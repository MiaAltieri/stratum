...
class StratumIndex:
    ...
    def __enter__(self) -> "StratumIndex":
        try:
            ...
            # WAL journal mode
            # WAL mode persists in the database file itself, so you only need to set it once — but
            # setting it on every connect is harmless and ensures it's always active. Place it
            # before any other operations so all subsequent reads/writes use WAL
            # from the start.
